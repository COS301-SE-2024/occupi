...
const RecommendationsModal = ({
  ...
  const [recommendations, setRecommendations] =
    useState<RecommendationsResponse | null>(null); // Store recommendations
  const [loading, setLoading] = useState(false); // Track loading state
  const [hasFetched, setHasFetched] = useState(false); // Track if data is already fetched

  // States for typing effect for both titles
  ...
  const messageToday = "These are the recommended times to go to the office:";
  const messageNext7Days = "Next 7 Days Recommendations:";
  ...
  // Simulate fetching recommendations (if not already fetched)
  useEffect(() => {
          ...
          try {
            ...
            const response = {
              Best_Times: [
                {
                  Hour: 6,
                  Predicted_Attendance_Level: "900-1200",
                  Predicted_Class: 3,
                ...
                {
                  Hour: 16,
                  Predicted_Attendance_Level: "300-600",
                  Predicted_Class: 1,
                  Recommendation: "Best time to go",
                },
                {
  ...
  const handleTabSwitch = (tab: "today" | "next7days") => {
    ...
    if (tab === "today" && typedMessageToday === "") {
      setTypingIndexToday(0); // Reset typing index for today
    }
    if (tab === "next7days" && typedMessageNext7Days === "") {
            ...
            <div className="relative overflow-hidden min-h-32">
              <div
                ...
                <div className="w-full flex-shrink-0">
                  ...
                  <div>
                    {/* Add content for next 7 days here */}
                    <p>Content for the next 7 days will be displayed here.</p>
                  </div>
                </div>
              </div>
            </div>
