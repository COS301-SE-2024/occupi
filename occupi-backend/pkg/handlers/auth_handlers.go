...
// common handler logic for sending OTP
func handlePasswordReset(ctx *gin.Context, appsession *models.AppSession, email string) {
    ...
    if exists := database.EmailExists(ctx, appsession.DB, sanitizedEmail); !exists {
        ctx.JSON(http.StatusBadRequest, utils.ErrorResponse(
            http.StatusBadRequest,
            "Email not registered",
            constants.InvalidAuthCode,
    ...
    success, err := database.AddOTP(ctx, appsession.DB, sanitizedEmail, otp)
    if err != nil {
        ctx.JSON(http.StatusInternalServerError, utils.InternalServerError())
